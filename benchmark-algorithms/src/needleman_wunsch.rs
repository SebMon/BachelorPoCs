...
pub fn calculate_score(x: &Vec<char>, y: &Vec<char>) -> (Vec<char>, Vec<char>,) {
    ...
    let mut f: Vec<isize> = vec![0; (ny + 1) * (nx + 1)];

    // Fill top row and left most column
    for i in 0..nx+1 {
        //f[i][0] = GAP_PENALTY * -(i as isize);
        f[i * (ny + 1) + 0] = GAP_PENALTY * -(i as isize);
    }
    for i in 0..ny+1 {
        //f[0][i] = GAP_PENALTY * -(i as isize);
        f[0 * (ny + 1) + i] = GAP_PENALTY * -(i as isize);
    }

    //let mut p = vec![vec![0; ny + 1]; nx + 1];
    let mut p = vec![0; (ny + 1) * (nx + 1)];
    for i in 0..nx+1 {
        //p[i][0] = 3;
        p[i * (ny + 1) + 0] = 3;
    }
    ...
    for i in 0..nx {
        for j in 0..ny {
            if x[i] == y[j] {
                ...
                t[0] = f[i * (ny + 1) + j] + MATCH_AWARD;
            } else {
                //t[0] = f[i][j] -  MISMATCH_PENALTY;
                t[0] = f[i * (ny + 1) + j] -  MISMATCH_PENALTY;
            }
            //t[1] = f[i][j + 1] - GAP_PENALTY;
            t[1] = f[i * (ny + 1) + (j + 1)] - GAP_PENALTY;
            //t[2] = f[i + 1][j] - GAP_PENALTY;
            t[2] = f[(i + 1) * (ny + 1) + j] - GAP_PENALTY;

            let max_value = *t.iter().max().unwrap();
            //f[i + 1][j + 1] = max_value;
            f[(i + 1) * (ny + 1) + (j + 1)] = max_value;
    ...
    while i > 0 || j > 0 {
        if [2, 5, 6, 9].contains(&p[i * (ny + 1) + j]) {
            rx.push(x[i - 1]);
            ry.push(y[j - 1]);
            i -= 1;
            j -= 1;
        } else if [3, 5, 7, 9].contains(&p[i * (ny + 1) + j]) {
            rx.push(x[i - 1]);
            ry.push('-');
            i -= 1;
        } else if [4, 6, 7, 9].contains(&p[i * (ny + 1) + j]) {
            rx.push('-');
            ry.push(y[j - 1]);
            j -= 1;
        }
