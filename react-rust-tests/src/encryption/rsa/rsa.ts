interface RSAModuloExponentSet {
  modulo: Uint8Array;
  exponent: Uint8Array;
}
...
export async function RSAencrypt(
  ...
  const keyLength = moduloExponentSet.modulo.length;
  ...
  for (let i = 0; i < blocks; i++) {
    output.set(
      await RSAES_OAEP_ENCRYPT(
        moduloExponentSet,
        input.slice(i * blockLength, (i + 1) * blockLength)
      ),
      i * keyLength
    );
  }
...
export async function RSAdecrypt(
  ...
  for (let i = 0; i < blocks - 1; i++) {
    output.set(
      await RSAES_OAEP_DECRYPT(
        moduloExponentSet,
        input.slice(i * keyLength, (i + 1) * keyLength)
      ),
      i * blockLength
    );
  }
...
async function RSAES_OAEP_ENCRYPT(
  key: RSAModuloExponentSet,
  M: Uint8Array
): Promise<Uint8Array> {
...
async function RSAES_OAEP_DECRYPT(
  key: RSAModuloExponentSet,
  C: Uint8Array
): Promise<Uint8Array> {
  const k = key.modulo.length;
...
function I2OSP(input: bigint, xLen: number): Uint8Array {
  ...
  for (let i = 1; i <= xLen; i++) {
    out[xLen - i] = Number(
      (input % BigInt(256) ** BigInt(i)) / BigInt(256) ** BigInt(i - 1)
    );
  }
  return out;
}
